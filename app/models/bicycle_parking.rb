class BicycleParking < ActiveRecord::Base
  ...
  START_DIAMETER = 0.5
  MAX_DIAMETER = 5
  SEARCH_PARAMS = { min_results: 3, start_diameter: 0.5, max_diameter: 0.5 }
  ...
  def self.search_within(lat, long, min_results=1, results=[],  diameter=0.1, inc_diameter=0.1, max_diameter=20)
    ...
    search_within lat, long, min_results, nearby_parking, diameter + inc_diameter, max_diameter
  end

  def self.search(lat, long, max_results)
    nearby_parking = search_within lat, long, MIN_RESULTS, [], START_DIAMETER, START_DIAMETER, MAX_DIAMETER
